...
public class SocketFactory extends javax.net.ssl.SSLSocketFactory{
    ...
    public SocketFactory(SocketFactoryOptions options) throws KeyStoreException, NoSuchAlgorithmException, IOException, KeyManagementException, java.security.cert.CertificateException, UnrecoverableKeyException {
        ...
        if(options.hasCaCrt()) {
            ...
            Log.v(TAG,"Certificate Owner: " + ca.getSubjectDN().toString());
            Log.v(TAG, "Certificate Issuer: " + ca.getIssuerDN().toString());
            Log.v(TAG,"Certificate Serial Number: " + ca.getSerialNumber().toString());
            Log.v(TAG,"Certificate Algorithm: " + ca.getSigAlgName());
            Log.v(TAG,"Certificate Version: " + ca.getVersion());
            Log.v(TAG,"Certificate OID: " + ca.getSigAlgOID());
            Enumeration<String> aliasesCA = caKeyStore.aliases();
            for (; aliasesCA.hasMoreElements(); ) {
                String o = aliasesCA.nextElement();
                Log.v(TAG, "Alias: " +o+ " isKeyEntry: " + caKeyStore.isKeyEntry(o) +" isCertificateEntry: "+caKeyStore.isCertificateEntry(o));
            }
